...
const IndexRoute: React.FC<Props> = ({ clientes }) => {
  ...
  clientesConRetornoBajo = clientes.filter(
    (cliente) =>
      cliente["Retorno"] < 37 &&
      cliente["Estado"] != "down" &&
      cliente["Calle"] != ""
  ...
  clientesConRetornoAlto = clientes.filter(
    (cliente) =>
      cliente["Retorno"] > 50 &&
      cliente["Estado"] != "down" &&
      cliente["Calle"] != ""
  ...
  clientesConErroresDs = clientes.filter(
    (cliente) =>
      (cliente["Ds FecPre"] != "0" || cliente["Ds FecPost"] != "0") &&
      cliente["Estado"] != "down" &&
      cliente["Calle"] != ""
  );
  cuadrasConErroresDs = cuadrasFueraDeRango(clientes, clientesConErroresDs);
  clientesConErroresUs = clientes.filter(
    (cliente) =>
      (cliente["Us FecPre"] != "0" || cliente["Us FecPost"] != "0") &&
      cliente["Estado"] != "down" &&
      cliente["Calle"] != ""
  );
  cuadrasConErroresUs = cuadrasFueraDeRango(clientes, clientesConErroresUs);
  clientesConDsTiltElevado = clientes.filter(
    (cliente) =>
      parseFloat(cliente["Tilt"]) > 10 &&
      cliente["Estado"] != "down" &&
      cliente["Calle"] != ""
  );
  cuadrasConDsTiltElevado = cuadrasFueraDeRango(
    clientes,
    clientesConDsTiltElevado
  );
  clientesCaidos = clientes.filter(
    (cliente) => cliente["Estado"] == "down" && cliente["Calle"] != ""
  );
  cuadrasCaidas = cuadrasFueraDeRango(clientes, clientesCaidos);

  imprimirEnPAgina = clientes.map((cliente) => (
    <RecorrerLaLista key={cliente["ID"]} value={cliente["Calle"]} />
  ));
  return (
    <div>
      <Stack
      ...
      >
        <Text fontWeight="500">
          Recorda cargar los dato del nodo en la planilla y recargar la pagina
        </Text>
        <Button
          isExternal
          as={Link}
      ...
      {Boolean(cuadrasConDirectaBaja.length) && (
        <Grid>
          ...
          >
            <Text fontWeight="500">
              {" "}
              Se verifican las siguientes cuadras del nodo{" "}
              {clientes[2]["Nodo cmts"]} con niveles de directa bajo:{" "}
            </Text>
        ...
        <Grid>
          ...
          >
            <Text fontWeight="500">
              {" "}
              Se verifican las siguientes cuadras del nodo{" "}
              {clientes[2]["Nodo cmts"]} con niveles de directa elevado:{" "}
            </Text>
        ...
        <Grid>
          ...
          >
            <Text fontWeight="500">
              {" "}
              Se verifican las siguientes cuadras del nodo{" "}
              {clientes[2]["Nodo cmts"]} con niveles de retorno bajo:{" "}
            </Text>
        ...
        <Grid>
          ...
          >
            <Text fontWeight="500">
              {" "}
              Se verifican las siguientes cuadras del nodo{" "}
              {clientes[2]["Nodo cmts"]} con niveles de retorno elevado:{" "}
            </Text>
        ...
        <Grid>
          ...
          >
            <Text fontWeight="500">
              {" "}
              Se verifican las siguientes cuadras del nodo{" "}
              {clientes[2]["Nodo cmts"]} con errores Ds:{" "}
            </Text>
        ...
        <Grid>
          ...
          >
            <Text fontWeight="500">
              {" "}
              Se verifican las siguientes cuadras del nodo{" "}
              {clientes[2]["Nodo cmts"]} con errores Us:{" "}
            </Text>
        ...
        <Grid>
          ...
          >
            <Text fontWeight="500">
              {" "}
              Se verifican las siguientes cuadras del nodo{" "}
              {clientes[2]["Nodo cmts"]} con Ds Tilt eleevado:{" "}
            </Text>
...
function cuadrasFueraDeRango(clientes, clientesConProblemasDeNiveles) {
  ...
  cuadrasUnicas.forEach((cuadra) => {
    let porcentajeDeCuadraAfectada =
      clientesConProblemasDeNiveles.filter(
        (cliente) => cliente["Calle"] == cuadra
      ).length /
      clientes.filter((cliente) => cliente["Calle"] == cuadra).length;
